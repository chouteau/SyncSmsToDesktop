...
namespace SmsSyncMaui.Services
{
    public class WebSocketServerManager
    {
        ...
        public event EventHandler<SmsEntry>? NewSmsReceived;
        public event EventHandler<List<SmsEntry>>? HistoryReceived;
        ...
        private async Task HandleClientAsync(TcpClient client, System.Threading.CancellationToken token)
        {
            ...
            try
            {
                ...
                using var webSocket = System.Net.WebSockets.WebSocket.CreateFromStream(
        ...
        private void OnMessageReceived(string messageJson)
        {
            try
            {
                ...
                {
                    case "sms_history":
                        var history = JsonSerializer.Deserialize<List<SmsEntry>>(message.Payload);
                        if (history != null)
                        {
                    ...
                    case "new_sms":
                        var newSms = JsonSerializer.Deserialize<SmsEntry>(message.Payload);
                        if (newSms != null)
                        {
                            Log($"Reçu nouveau SMS de {newSms.Address}.");
                            SaveMessagesToDb(new List<SmsEntry> { newSms });
        ...
        private void SaveMessagesToDb(List<SmsEntry> messages)
        {
            try
            {
                ...
                foreach (var msg in messages)
                {
                    ...
                    else
                    {
                        ...
                        {
                            existing.AttachmentBase64 = msg.AttachmentBase64;
                        }
                        if (!string.IsNullOrEmpty(msg.AttachmentMimeType))
                        {
                            existing.AttachmentMimeType = msg.AttachmentMimeType;
                        }
                    }
                }
                db.SaveChanges();
